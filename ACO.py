#https://github.com/Norm723/ACO-n-queens/blob/master/ACO.py
# -*- coding: utf-8 -*-
"""
...
class AntforTSP(object) :
    ...
    def evalTour(self, path, contradictions):
        ...
        if res == 0:
            ...
            f = open("memory_ACO_N_queens.txt", "a")
            f.write(str(self.dim)+": " + str(tracemalloc.get_traced_memory()[1])+"\n" )
            f.close()
            ...
            print("--- %s seconds ---" % (t)) 
            f = open("time_ACO_N_queens.txt", "a")
            f.write(str(self.dim)+": " + str(t)+"\n" )
            f.close()
            exit(0)
        return res
#
